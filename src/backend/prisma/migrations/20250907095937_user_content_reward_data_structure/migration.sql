...
INSERT INTO "content_reward" ("content_id", "item_id", "updated_at")
SELECT c."id" as content_id, i."id" as item_id, CURRENT_TIMESTAMP
FROM "content" c
CROSS JOIN "item" i
ON CONFLICT ("content_id", "item_id") DO NOTHING;

-- 더보기 보상은 레이드 컨텐츠에 대해서만 생성함
INSERT INTO "content_see_more_reward" ("content_id", "item_id", "updated_at")
SELECT c."id" as content_id, i."id" as item_id, CURRENT_TIMESTAMP
FROM "content" c
CROSS JOIN "item" i
JOIN "content_category" cc ON c."content_category_id" = cc."id"
WHERE cc."name" IN ('에픽 레이드', '카제로스 레이드', '강습 레이드', '군단장 레이드')
ON CONFLICT ("content_id", "item_id") DO NOTHING;
